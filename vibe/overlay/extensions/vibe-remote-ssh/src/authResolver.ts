        ...
        return vscode.window.withProgress({
            ...
            try {
                ...
                const installResult = await installCodeServer(
                    ...
                    { tarballSetting: remoteSSHconfig.get<string>('vibeServerTarball'), appRoot: vscode.env.appRoot, report: message => progress.report({ message }) } // vibe: upload install mode
                );
                ...
                if (context.resolveAttempt === 1) {
                    ...
                    const retry = 'Retry';
                    const result = await vscode.window.showErrorMessage(`Could not establish connection to "${sshDest.hostname}"`, { modal: true, detail: e instanceof ServerInstallError ? e.message : undefined }, closeRemote, retry); // vibe: detail, it tells what to do
