...
import { nodeLauncherSystem, runProgram, type LauncherSystem, type RunOptions } from '../src/host/launcher/system.ts';
import { MAX_OUTPUT_BYTES } from '../src/model/launcher/runtime.ts';
import { DEFAULT_LAUNCHER_SETTINGS, childEnvOf, launcherSettingsOf, type LauncherSettings } from '../src/model/launcher/settings.ts';
import { healthOf, startFakeDaemon, type FakeDaemon } from './fakes/fake-daemon.ts';

const FAKES = path.join(import.meta.dirname, 'fakes');
const fake = (name: string) => path.join(FAKES, name);
...
async function bench(t: TestContext, scenario: string, options: { daemon?: boolean; routed?: boolean; launcher?: 'stopped' | 'running-visible'; env?: Record<string, string> } = {}): Promise<Bench> {
