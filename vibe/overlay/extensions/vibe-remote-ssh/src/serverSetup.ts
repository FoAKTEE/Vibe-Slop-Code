...
import { ensureServerInstalled, ServerUploadProgress } from './vibe/serverUpload'; // vibe:
...
export async function installCodeServer(
    ...
    vibe: { readonly tarballSetting: string | undefined; readonly appRoot: string; readonly report: ServerUploadProgress } // vibe: upload install mode
    ...
    // vibe: upload install mode. The server is a build on this machine that travels over the
    // connection, the host downloads nothing. A build from sources has no commit of its own:
    // the build that is uploaded tells which one the host runs.
    if (platform !== 'windows') {
        try {
            vscodeServerConfig.commit = await ensureServerInstalled(conn, {
                productCommit: vscodeServerConfig.commit,
                serverApplicationName: vscodeServerConfig.serverApplicationName,
                serverDataDir: customInstallPath ? customInstallPath.replace(/^~(?=\/|$)/, '$HOME') : `$HOME/${vscodeServerConfig.serverDataFolderName}`,
                hasDownloadUrlTemplate: !!(serverDownloadUrlTemplate || vscodeServerConfig.serverDownloadUrlTemplate),
                tarballSetting: vibe.tarballSetting,
                appRoot: vibe.appRoot
            }, logger, vibe.report);
        } catch (e) {
            throw new ServerInstallError(e instanceof Error ? e.message : String(e));
        }
    }
